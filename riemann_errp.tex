...
error detection, event expectancy, object recognition etc. -- as seen
in the Event-Related Potential (ERP) waveforms provide a tremendous potential for brain computer interfacing. 
As an illustration, when an error committed by
the user or machine is observed, an Error-Related Potential (ErrP) is elicited
and can be used to correct it.
...
ERP can be successfully detected in various paradigms under controlled conditions
using directly the EEG channel activity over the time-window of interest \cite{blankertz_single-trial_2011}.
When facing real world conditions, our cognitive processing may be challenged
by diverse stimuli and complex environment.
...
An alternative way to detect ERP is to use features based on spatial covariance matrices
...
to address the ERP latency variability challenge of real world BCI application.
In controlled conditions directly using EEG spatial covariance matrix
is not optimal as temporal dynamics is lost.
Therefore, it is advisable to augment the matrix
with the covariances between single trial and average ERP
of one of the classes \cite{congedo_new_2013,barachant_plug&play_2014}.
Under ERP latency variability challenge, however, 
augmented covariance matrices may have the same issue as
waveforms. Thus, we need to consider simple channel-based covariance matrices.
Covariance matrices can be used directly \cite{tomioka_classifying_2007} as features, 
although it is not optimal. It is shown that treating them under Riemannian geometry framework
...
natural environment.
Secondly, we study the classifiers transfer between
different ErrP protocols. This corresponds to reducing BCI calibration time
as requested by the end users.
...
\begin{figure}[!t]
    \includegraphics[trim={0cm 0cm 0 0cm},clip,width=0.5\columnwidth]{../images/FCz_all_diff_Topo_compact.pdf}
    \includegraphics[trim={0cm 0cm 0 0cm},clip,width=0.5\columnwidth]{../images/FCz_all_diff_noise300.pdf}
...
\color{red}{On the right: noise level 300 ms}}
\label{fig:FCz}
\end{figure}
...
outliers \cite{parra_recipes_2005}. The regularization parameter is chosen by 4-fold cross-validation. The data are split to preserve the
...
The authors of the used dataset reported high performance with a linear classifier.
...
The overall trend is the reduction in performance with the increase in temporal jitter for all features.
...
\begin{figure*}[!t]
    ...
    The right-most matrix provides p-values obtained with ANOVA when comparing features
...
carry sufficient information for high performance above 75 AUC points for all three experiments.
Waveform features outperform covariance features yet their combination results
...
for considerable jitter levels above 200 ms or latency differences
...
performance as compared to individual feature sets.
...
%without recalibrating the BCI system.
